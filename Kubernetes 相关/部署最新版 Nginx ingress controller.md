...
```
# 补充
由于 yaml 中使用镜像源在国外，国内很难拉取成功，所以下面附加一份当前（2024-1-17）已经修改好的镜像源为阿里云 acr 的 yaml：
```yaml
apiVersion: v1
kind: Namespace
metadata:
  ...
  template:
    ...
    spec:
      ...
      - args:
        ...
        image: registry.cn-shenzhen.aliyuncs.com/zze/ingress-nginx-controller:v1.9.5
      ...
      - args:
        ...
        - name: POD_NAMESPACE
          valueFrom:
            fieldRef:
              fieldPath: metadata.namespace
        image: registry.cn-shenzhen.aliyuncs.com/zze/ingress-nginx-kube-webhook-certgen:v20231011
        imagePullPolicy: IfNotPresent
        name: create
        securityContext:
          allowPrivilegeEscalation: false
